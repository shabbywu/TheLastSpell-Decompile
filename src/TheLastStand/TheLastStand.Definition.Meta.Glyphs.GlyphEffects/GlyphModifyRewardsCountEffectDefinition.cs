...
public class GlyphModifyRewardsCountEffectDefinition : GlyphEffectDefinition
{
	...
	public override void Deserialize(XContainer container)
	{
		...
		if (val != null)
		{
			GlyphDefinition.AssertIsTrue(int.TryParse(val.Value.Replace(base.TokenVariables), out var result), "Could not parse NightRewardsModifier into an int in ModifyRewardsCount");
			NightRewardsModifier = result;
		}
		XAttribute val2 = ((XElement)obj).Attribute(XName.op_Implicit("ProdRewardsModifier"));
		if (val2 != null)
		{
			GlyphDefinition.AssertIsTrue(int.TryParse(val2.Value.Replace(base.TokenVariables), out var result2), "Could not parse ProdRewardsModifier into an int in ModifyRewardsCount");
