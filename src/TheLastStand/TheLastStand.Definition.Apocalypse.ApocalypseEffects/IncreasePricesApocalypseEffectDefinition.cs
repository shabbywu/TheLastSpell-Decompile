...
public class IncreasePricesApocalypseEffectDefinition : ApocalypseEffectDefinition
{
	...
	public override void Deserialize(XContainer container)
	{
		if (container != null)
		{
			...
			{
				CLoggerManager.Log((object)("An Apocalypse's IncreasePrices Effect " + ((Definition)this).HasAnInvalid("E_PriceModifierType", val.Value)), (LogType)0, (CLogLevel)2, true, "StaticLog", false);
			}
			Type = result;
			XAttribute val2 = ((XElement)obj).Attribute(XName.op_Implicit("Value"));
			if (!int.TryParse(val2.Value, out var result2))
			{
				CLoggerManager.Log((object)("An Apocalypse's IncreasePrices Effect " + ((Definition)this).HasAnInvalidInt(val2.Value)), (LogType)0, (CLogLevel)2, true, "StaticLog", false);
			}
			Value = result2;
		}
	}
